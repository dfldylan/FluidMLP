...
class SlowFluidNet(tf.keras.Model):
    def __init__(self, *args, **kwargs):
        ...
        self.global_step = tf.Variable(initial_value=0, dtype=tf.int64, name='global_step', trainable=False)
    ...
    def _each_particle(self, inputs):  # (m1, cp1)
        ...
        neighbor_particles = tf.gather(self.data, mask)
        t = neighbor_particles[:, :3] - center_particle[:3]
        neighbor_particles = tf.concat((t, neighbor_particles[:, 3:]), axis=1)
        fluid_part = tf.gather(neighbor_particles, tf.equal(neighbor_particles[:, 6], tf.constant(0)))[:, :6]
        solid_part = tf.gather(neighbor_particles, tf.equal(neighbor_particles[:, 6], tf.constant(1)))[:, :3]
        neigh_fluid = tf.concat((fluid_part, tf.tile(center_particle[:, 3:], (fluid_part.shape[0], 1))),
                                axis=1)  # (-1, 9)
        neigh_solid = tf.concat((solid_part, tf.tile(center_particle[:, 3:], (solid_part.shape[0], 1))),
                                axis=1)  # (-1, 6)
        x = self.fluid_fcn0(neigh_fluid)
        x = self.fluid_fcn1(x)
        x = self.fluid_fcn2(x)
